...
FirstSecMonth %>% ggplot(aes(x=reorder(RegionName,Years),y=Years ,fill=Size))+geom_bar(stat='identity',position='dodge', width=.9) + theme(axis.text.x=element_text(angle=45,hjust=1))  +xlab("State Name")+ylab("Years on Market")+guides(fill=guide_legend(title="Population of state"))
```
