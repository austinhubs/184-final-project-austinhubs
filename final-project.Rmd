...
MinMaxMeanPerState <- 
  SalePricesState %>%
  mutate(MinimumPrice = apply(JustMonthsState,1,min,na.rm=TRUE)) %>%
  mutate(MaximumPrice = apply(JustMonthsState,1,max,na.rm=TRUE)) %>%
  mutate(MeanPrice = apply(JustMonthsState,1,mean,na.rm=TRUE)) %>%
...
```{r}
ggplot( data = MinMaxMeanPerState, aes(x = MeanPrice,fill="All US States")) + geom_density(adjust = 0.38,alpha=.5)+labs(title = "")+geom_density(aes(x = MeanPrice,fill="Top 626 Cities"),TotalInfo,adjust = 0.38,alpha=.5)+guides(fill=guide_legend(title="Data"))
```
