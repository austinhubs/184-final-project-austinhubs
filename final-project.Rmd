...
```{r}
Justdata <- Saleprices
Justdata$SizeRank <- NULL
Justdata$RegionID <- NULL
Justdata$RegionName <- NULL
Maxarr <- apply(Justdata,1,max,na.rm=TRUE)
Minarr <- apply(Justdata,1,min,na.rm=TRUE)
Minarr
Saleprices %>%
  mutate(Maximum = Maxarr) %>%
  mutate(Minimum = Minarr)
